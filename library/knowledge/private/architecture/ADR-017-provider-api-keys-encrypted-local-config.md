...
Amendment note (2026-08-21): the provider-key class this ADR covers is not limited to the providers named in its Context section (ElevenLabs, OpenAI, Gemini, Deepgram, R2). Every later-added Waggle-internal provider key follows the same rule set: the phase 3/4 keys (vision-QA key for prd-011, explorer LLM key for prd-015, STT key and intent-parser LLM key for prd-016, HeyGen and Tavus keys for prd-017) are provider keys under this ADR, entered via the GUI store, never stored in any project directory, with env-ref resolution on the CLI/CI path. This note records scope clarification only; the decision text is unchanged.
