...
Scope note (added 2026-08-21, HANDOFF-4 Wave 0): the provider-key surface covers ALL Waggle-internal provider keys per ADR-017's amendment note, which is three packages, not one: `packages/narrate` (ElevenLabs), `packages/ingest` pre-draft LLM keys (OPENAI_API_KEY / ANTHROPIC_API_KEY per `packages/ingest/src/predraft/env-config.ts`, injected today via `runIngest`'s `predraftEnv` option), and `packages/share` R2 keys (WAGGLE_R2_* per prd-008 AC3). The GUI store serves all three; the env-ref path stays authoritative for each on CLI/CI.
...
- A `KeySource` injection seam on provider-consuming packages (`packages/narrate`, `packages/ingest` pre-draft, `packages/share`) so desktop supplies decrypted values in-process at call time without touching adapter logic. Implementation shape: KeySource as the injectable env-record interface all three packages already accept (narrate's provider config, runIngest's `predraftEnv`, share's R2 env record) - the desktop implementation of KeySource returns the same record shape the env-ref path builds, so adapters see one contract.
- Headless-Linux fallback per ADR-017: machine-local key file outside every project directory when no OS secret store is available (per the 2026-08-21 Linux ruling, this fallback still ships because from-source Linux builds exist without the desktop app; it is the CLI-side fallback ADR-017 mandates, not a packaging commitment).
...
- `packages/ingest` - pre-draft provider config accepts the same injected record (it already accepts `predraftEnv`; the KeySource implementation fills it on the desktop path)
- `packages/share` - R2 config accepts the same injected record (env-record shape per prd-008 AC3)
