...
- **Live-progress channel ownership (added 2026-08-21):** today ingest runs synchronously inside the `meta.json` POST (the final upload call in `apps/extension/src/lib/upload-client.ts`); the upload's HTTP response cannot carry minutes of progress. The progress channel is the existing `api/watch` SSE surface (`apps/studio/src/routes/api/watch/+server.ts`), extended with ingest job events; sub-PRD i owns the event schema and the job-runner pattern, and f's auto-processing is one producer on that channel. Ownership is explicit so f and i cannot each invent a channel.
- **`documentPictureInPicture` user-gesture constraint (added 2026-08-21):** the PiP window can only be opened from a user gesture (a click in the recorded tab's document); an extension-initiated flow that starts from the toolbar action must open the PiP window as part of that same gesture's handler chain, or fall back to the popup surface. The countdown is the designed mitigation: it guarantees the gesture-to-capture gap contains no user movement the video would miss, and the implementation detail (gesture budget) is asserted in the e2e, not assumed.
