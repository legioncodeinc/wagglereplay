...
**Acceptance criteria:**
- AC-18a.2.1 Given the BrowserWindow configuration, then `webPreferences` sets `contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`, and `webviewTag: false`, asserted by a unit test over the config object.
...
- AC-18a.2.5 Given the window, then `will-navigate` is denied for every target except the app's own loopback origin, and `setWindowOpenHandler` denies all new windows/popups (navigation denial; a compromised page cannot navigate out of the app surface).
- AC-18a.2.6 Given any IPC message arriving at the main process, then the sender is validated (only the app's own renderer webContents id may invoke bridge channels) before any handler runs; messages from other frames or processes are dropped and logged.
- AC-18a.2.7 Given the packaged (production) build, then DevTools are not openable (no keyboard shortcut, no menu item; `webContents` devtools APIs gated to dev mode), asserted in the packaged smoke e2e.
...
- **Server hosting:** import the adapter-node server output in-process rather than spawning `node build/index.js` as a child - ADR-020's "no sidecar runtime" wording. The importable entry is adapter-node's `build/handler.js`, which exports the `handler` (an http-request-listening function) without the self-contained `index.js` bootstrap; the desktop main wraps that handler in its own `node:http` server on `127.0.0.1` with explicit port config (pinned 4310 per sub-PRD b). If the emitted handler cannot be imported cleanly, extract a thin programmatic server entry inside `apps/studio` that both the CLI and the desktop main import. Do not fork the server implementation.
...
- **Electron pin (decided 2026-08-21, decision 9):** pin major 43 (current stable; embedded Node 24.18 satisfies the Node 24+ floor; Electron supports majors 41-43 as of pinning day). Record the exact version and its embedded Node in `apps/desktop/package.json` and ADR-020's engine-floor consequence; sub-PRD g turns the floor check into a build failure.
...
- **Open question resolved (2026-08-21):** which Electron major to pin - major 43, embedded Node 24.18 (see Technical Considerations); recorded for ADR-020's engine-floor consequence.
