...
- Engine-floor build check: fail `dist` if `process.versions.node` of the pinned Electron is below the repo's floor.
- Resource strategy for Playwright Chromium and ffmpeg: bundled as app resources, or a documented first-run download — decided here, delivered here.
...
- **Open question:** whether artifacts should also build in CI on tagged pushes. Recommend yes (build-only, no publish) — decide with the PRD-012 CI philosophy rather than unilaterally.
