...
| T2 | Environment resolution and fill-time injection. | ✅ | `packages/replay/src/credentials/binding.ts:143-265`; `packages/replay/src/steps/act.ts:181-196`; `packages/replay/src/credentials/index.ts:1-9`; `packages/replay/test/public-surface.test.ts:1-26` (citation corrected 2026-08-21, Run 4: this row originally cited `packages/replay/test/index.test.ts:4-41`, which contains only preset-registry tests; no barrel guard existed at QA time. The code was correct; the guard now lives in public-surface.test.ts, added by the Run 4 guardrail pass) | Value-bearing resolution is callback-only and its factory is absent from the public replay credentials barrel. |
...
- `packages/replay/test/public-surface.test.ts` (A, Run 4 guardrail pass 2026-08-21), verifies the public surface does not expose the resolver factory. Correction: this list originally credited `packages/replay/test/index.test.ts` with that check; that file holds preset-registry tests only. The barrel omission was correct in code but unguarded until public-surface.test.ts.
