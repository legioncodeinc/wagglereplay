...
Waggle: open source, local-first toolchain that records a web app walkthrough once (Chrome extension: pixels + click/route/element telemetry), stores it as a git-committable Walkthrough IR, and regenerates narrated demo videos from it at any aspect ratio via Playwright replay + ffmpeg composition. AGPL-3.0. Personal-first project by Mario Aldayuz (Legion Code Inc). Pre-alpha: planning library seeded, no code yet.

## Read first, always

1. `library/knowledge/private/architecture/` holds 15 accepted ADRs. They are locked decisions; do not relitigate them in PRs or PRDs. Supersede via a new ADR only.
2. `library/requirements/backlog/` holds the 17-PRD build plan (phases 1 to 4). PRD section 0 dependencies dictate build order; never build a PRD whose dependencies are unmet.
